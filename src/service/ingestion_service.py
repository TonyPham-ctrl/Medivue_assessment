...
class IngestionService:
    ...
    async def process(self, payload: ReadingWrapper):
        ...
        if validation_status != IngestionStatus.SUCCESS:
            alert_message = await self._dispatch_alert(ValidationAlert(payload.patient_id, str(payload.reading.recorded_at), validation_message))
        ...
        if alert_type != AlertType.NORMAL:
            ...
            response["alert"] = await self._dispatch_alert(GlucoseAlert(payload.patient_id, str(payload.reading.recorded_at), alert_type))
        else:
            response["threshold"] = alert_type.value

        if validation_service.check_battery(payload):
            response["battery_alert"] = await self._dispatch_alert(BatteryAlert(payload.patient_id, str(payload.reading.recorded_at), payload.reading.battery_pct))
