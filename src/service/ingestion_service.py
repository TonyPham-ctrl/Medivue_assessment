...
class IngestionService:
    ...
    async def process_reading(
        ...
        -  the same (patient_id, device_id, recorded_at) triple is only
          processed once per session.
        -  only one ACTIVE late-reading alert is maintained per device
        ...
        if lateness_seconds <= LATE_READING_THRESHOLD_MINUTES * 60:
            # silently resolve any active late alert for this device
            alert_state_tracker.update_state(pid, "late_reading", AlertType.NORMAL, device_id=did)
            return None

        lateness_minutes = int(lateness_seconds / 60)

        if not alert_state_tracker.should_fire(pid, "late_reading", AlertType.LATE_READING, arrival_time, device_id=did):
            alert_state_tracker.update_state(pid, "late_reading", AlertType.LATE_READING, device_id=did)
            return None

        alert = LateReadingAlert(pid, did, recorded_at, arrival_time, lateness_minutes)
        alert_state_tracker.record_fired(pid, "late_reading", AlertType.LATE_READING, arrival_time, device_id=did)
