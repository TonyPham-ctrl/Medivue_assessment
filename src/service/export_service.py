...
class ExportService:
    ...
    def construct_reading_payload(self, reading: dict):
        ...
        summaries = []
        for r in reading[:N]:
            summaries.append(ReadingPayload(
                glucose_mgdl=r['glucose'],
    ...
    def construct_export_payload(self, patient_id: str, readings: List):        
        ...
        return ExportPayload(
            ...
            alert_status=most_recent_alert.type_label if most_recent_alert else "none",
    ...
    def process(self, patient_id: str):
        readings = storage_service.query_readings(patient_id)
        if not readings:
