...
def is_duplicate(self, device_id, recorded_at):
...
def validate_post_payload(payload):
    ...
    if payload.payload.glucose_mgdl <= 0:
        response_message += "glucose_mgdl must be a positive number \n"
        response_status = 400

    if not (0 <= payload.payload.battery_pct <= 100):
        response_message += "battery_pct must be between 0 and 100 \n"
        response_status = 400

    if payload.payload.signal_quality not in ["good", "poor", "degraded"]:
